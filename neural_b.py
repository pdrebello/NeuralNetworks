...
class Layer:
    def __init__(self, inputsize, outputsize,activation='sigmoid',lastlayer=False):
        ...
        #self.weights = np.random.randn(inputsize+1,outputsize)*np.sqrt(1/(inputsize))/255
        #self.bias = np.random.randn((outputsize))*np.sqrt(1/(inputsize))/255
...
Y = X[:,1024].reshape((X.shape[0],1))
Y = OneHotEncoding(Y,10).reshape(X.shape[0],10)
X = X[:,0:1024]
